...
class _Game(commands.Cog, name='Game'):
    ...
    async def coins_leaderboard(self, ctx: Context):
        ...
        async for entry in Game.find().sort('coins', -1):
            ...
            if mem == ctx.author:
                to_append = (
                    f'**{place} {mem.display_name} (YOU)**',
                    f'**{entry.coins:,}** {self.coin_emoji}'
                )
            else:
                to_append = (
                    f'{place} {mem.display_name}',
                    f'**{entry.coins:,}** {self.coin_emoji}'
                )
    ...
    @game_boss.command(name='start')
    @utils.is_owner()
    async def boss_start(self, ctx: Context):
        """Start the boss fight. This also means that the timer until the next boss fight will be reset."""

        await ctx.message.add_reaction(ctx.thumb)

        self.boss_fight.restart()

    @game_boss.command(name='stop')
    @utils.is_owner()
    async def boss_stop(self, ctx: Context):
        """Stops the task to start a boss fight."""

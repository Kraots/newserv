...
class IntroField(disnake.ui.Select['IntroFields']):
    ...
    async def callback(self, inter: disnake.MessageInteraction):
        ...
        value = value.lower()
        data: utils.Intro = await self.view.ctx.bot.db.get('intro', inter.author.id)
        ...
        if value == 'age':
            try:
                ...
                if new_data < 14 or new_data > 19:
                    if inter.author.id != self.view.ctx.bot._owner_id:
                        if any(r for r in utils.StaffRoles.all if r in (role.id for role in inter.author.roles)) is False:
                            entry: utils.Constants = await self.view.ctx.bot.db.get('constants')
                            if inter.author.id not in entry.ogs:
                                await self.delete_intro_message(data)
                                if new_data < 14:
                                    await utils.try_dm(
                                        inter.author,
                                        f'{self.view.ctx.denial} Sorry! You\'re too young for this server.'
                                    )
                                elif new_data > 19:
                                    await utils.try_dm(
                                        inter.author,
